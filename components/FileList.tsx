...
export function FileList({ userSession }: SharedClientProps) {
  const [files, setFiles] = useState<{ gen_file: string; gen_filename: string; audio_filename: string; }[]>([]);
  ...
  React.useEffect(() => {
    ...
    const subscription = channel
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'mas_generations' }, payload => {
        console.log('New file received!', payload);
        // Update the files state directly
        setFiles(prevFiles => [...prevFiles, payload.new as { gen_file: any; gen_filename: any; audio_filename: any; }]);
      })
      .subscribe();
