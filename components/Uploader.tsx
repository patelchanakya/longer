...
export default function Uploader({ userSession, channel }: { userSession: any, channel: any }) {
  ...
  const [userCredits, setUserCredits] = React.useState<number | undefined>(undefined);
  ...
  React.useEffect(() => {
    ...
    const subscription = channel
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'user_credits', filter: `user_id=eq.${userSession.id}` }, (payload: { record: any; }) => {
        ...
        {userSession && (
          ...
          >
            ...
            <div className="flex flex-col gap-2 w-full h-full justify-center px-10 text-black border-black ">
              <Slider name="slider" defaultValue={[sliderValue]} min={1} max={30} step={1} onValueChange={(value) => {
